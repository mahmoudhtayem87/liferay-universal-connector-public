...
const executionFolder = './dist/winos';
...
try {
    ...
    fs.writeFileSync(path.join(executionFolder, 'sea-config.json'), '{ "main": "index.js", "output": "sea-prep-win.blob" }');

    execSync('node --experimental-sea-config sea-config.json', {cwd: executionFolder});

    execSync('node -e "require(\'fs\').copyFileSync(process.execPath, \'server_win.exe\')"', {cwd: executionFolder});

    execSync('signtool remove /s server_win.exe ', {cwd: executionFolder});

    execSync('npx postject server_win.exe NODE_SEA_BLOB sea-prep-win.blob --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2', {cwd: executionFolder});
    ...
    fs.unlinkSync(path.join(executionFolder, 'sea-prep-win.blob'));
